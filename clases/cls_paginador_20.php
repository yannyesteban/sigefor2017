	...
	function control(){
		...
		for($i=$ini;$i<=$fin;$i++){
			if($i == $this->pagina){
				...
				$t->cell[0][$i+2-$ini]->text = $i;
				$t->cell[0][$i+2-$ini]->onclick = "enviar(false,$this->panel,'panel:$this->panel;pagina:$i;',0,'');";
			}// end if
		}// next
		//===========================================================
		...
		//===========================================================
		$t->cell[0][0]->onclick = "enviar(false,$this->panel,'panel:$this->panel;pagina:$pag_ini;',0,'');";
		$t->cell[0][1]->onclick = "enviar(false,$this->panel,'panel:$this->panel;pagina:$pag_ant;',0,'');";
		$t->cell[0][$fin+3-$ini]->onclick = "enviar(false,$this->panel,'panel:$this->panel;pagina:$pag_sig;',0,'');";
		$t->cell[0][$fin+4-$ini]->onclick = "enviar(false,$this->panel,'panel:$this->panel;pagina:$pag_fin;',0,'');";
		//===========================================================
		if ($this->pag_bloque < $this->nro_paginas){
			$ele = new cls_element_html("select");
			$ele->class = $this->clase_combo;
			$ele->onchange = "enviar(false,$this->panel,'panel:$this->panel;pagina:'+this.value+';',0,'');";
