...
define ("C_GRAFICO_ANCHO","700");
define ("C_GRAFICO_ALTO","300");
class cls_grafico extends cfg_grafico{
	var $path = C_PATH_GRAFICO;
	var $path_graph = "cache_graph/";
	...
	function control($grafico_x = ""){
		...
		include($this->path."clases/pData.php");   
		include($this->path."clases/pChart.php");
		...
		case 2:
			...
			$rgb=a_rgb($this->esc_color);
			$Test->drawPieGraph($DataSet->GetData(),$DataSet->GetDataDescription(),$this->ancho/2,$this->alto/2,$this->alto-$this->gra_margen*3,PIE_PERCENTAGE_LABEL,FALSE,$this->gra_trt_inclinacion,$this->gra_trt_altura,$this->gra_trt_separacion,$this->esc_decimales,$rgb[0],$rgb[1],$rgb[2]);
				if(!$this->ley_enmarcada){
			...
			$Test->drawPieLegend($ley_pos_x,$this->ley_pos_y,$DataSet->GetData(),$DataSet->GetDataDescription(),$rgb[0],$rgb[1],$rgb[2],$rgbs[0],$rgbs[1],$rgbs[2],$rgbt[0],$rgbt[1],$rgbt[2],!$this->ley_fnd_ocultar);
		...
		if(!$this->tit_oculto){	
			...
			$Test->drawTitle($this->tit_pos_x,$this->tit_pos_y,$this->titulo,$rgb[0],$rgb[1],$rgb[2],$this->tit_ancho,$this->tit_alto,$this->tit_sombra);
		}// end if
