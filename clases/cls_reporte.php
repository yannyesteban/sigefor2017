...
class cls_reporte extends cfg_reporte{
	...
	public $lineas = false;
	public $pag_informe = false;
	public $nro_pag_inf = 0;
	
	public $diagrama = "NO FOUND";
	...
	function rep_rapido(){
		...
		for($i=0;$i<$this->nro_campos;$i++){
			...
			if($this->totales!=""){
				...
				if(isset($this->es_sumar[$this->elem[$i]->nombre]) and $this->es_sumar[$this->elem[$i]->nombre]>0){
		...
		if($this->totales !=""){	
			...
			for($j=0;$j<$this->nro_campos;$j++){		
				...
				if(isset($this->tcampo[$this->elem[$j]->nombre]) and $this->tcampo[$this->elem[$j]->nombre]){
		...
		if(@($this->linea_actual % $this->lineas_pag==0) or $fin_informe){
			...
			if($this->mostrar_pagina($this->nro_pag_inf)){
				if($this->nro_pag_inf==1 or $this->nro_pag_inf==$this->pagina_ini){
					...
					$diagrama_x = str_replace("--salto_pagina--","",$this->diagrama);/*page-break-before:always*/
				}// end if
