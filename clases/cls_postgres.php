...
class cls_postgres{
	...
	function cls_conexion($servidor="", $usuario="", $password="", $bdatos="", $puerto="", $charset = false){
		...
		if(defined("C_BD_CHARSET")){
			pg_set_client_encoding($this->conexion, C_BD_CHARSET);
		}
		//pg_set_client_encoding($this->conexion, "UTF-8");
		//pg_set_client_encoding($this->conexion, "SQL_ASCII");
    }// end if
	//===========================================================
    function ejecutar($query_x=""){
		...
		if (substr_count(strtoupper(substr($this->query,0,10)),strtoupper("INSERT "))==0){
		}else{
		
			//$this->result = pg_query($this->query);
			///hr( pg_result_status($this->result));
			
			
			
			//hr(pg_last_oid($this->result));
		}
        ...
        if ($this->es_select($this->result)){
	        ...
	        if ($this->paginacion == C_SI 
                ...
                //$this->result = @pg_get_result($this->query." LIMIT $this->reg_pag OFFSET $this->reg_ini");
				
				pg_send_query($this->conexion,$this->query." LIMIT $this->reg_pag OFFSET $this->reg_ini");
				$this->result = pg_get_result($this->conexion);
			...
			if($this->tabla_insert){
				...
				$result = pg_get_result($this->conexion);					
				
				//$result = @pg_query($query_last_id);
				if($result){
					$rs=@pg_fetch_assoc($result);
					$this->insert_id = $rs["last_id"];
				}// end if
			}// end if
    ...
    function descrip_campos($result=""){
    	...
    	for ($i=0;$i< $this->nro_campos;$i++){
			...
			//$this->campo[$tabla][$campo]->longitud = (@pg_field_prtlen($result,$i)>0)?@pg_field_prtlen($result,$i):"";
			$this->campo[$tabla][$campo]->meta = $this->tipo_meta($this->campo[$tabla][$campo]->tipo);
			//hr($this->campo[$tabla][$campo]->meta,"yellow","black");
		...
		foreach ($this->tablas as $tabla => $v){
			...
			while ($rs = @pg_fetch_array($result)){	
				$campo = $rs["nombre"];
				if(!isset($this->campo[$tabla][$campo]) or !$this->campo[$tabla][$campo]->nombre){
					continue;
				}// end if
				...
				if(isset($this->campo[$tabla_x][$campo]) and $this->campo[$tabla_x][$campo]->meta == C_TIPO_D){
	...
	function es_select($result_x){
		if (substr_count(strtoupper(substr($this->query,0,10)),strtoupper("INSERT "))==0 and (substr($result_x."",0,8)=="Resource"
	...
	public function evalFiltros($q, $searchs, $fields, $quantifier = "%"){
		...
		foreach($searchs as $search){
			$search = $this->to_ascii($search);
			foreach($fields as $field){
				//$str .= (($str!="")?" OR ":"")."to_ascii(".$field."::varchar) ILIKE to_ascii('$quantifier$search$quantifier')";
				$str .= (($str!="")?" OR ":"")."".$field."::varchar ILIKE '$quantifier$search$quantifier'";
			}// next
		}
		$str = "(". $str. ")";
