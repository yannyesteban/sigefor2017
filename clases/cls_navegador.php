...
class cls_navegador{
	...
	function control($navegador_x=""){
		...
		//===========================================================
		//print_r($this->vses);exit;
		$this->usuario = $this->vses["SS_USUARIO"];
		//===========================================================
		$cn = &$this->conexion;
		$cn->query = "	SELECT DISTINCT
...
//hr($item[$i]->panel_actual);
	...
	function nav_normal(){
		...
		for($i=0;$i<$this->nro_items;$i++){
			...
			//print_r($item->even);
			if($item->even){
				foreach($item->even as $k => $v){
					//hr($v);
					//hr(htmlentities($v));
					$boton->$k .= ";". htmlentities($v);
				}
			}
		...
		if($prop = extraer_para($this->propiedades)){
			foreach($prop as $para => $valor){
				$t->cell[0][0]->$para = $valor;
			}// next
		}// end if

		if(isset($item) and $item->abrir_menu!=""){
